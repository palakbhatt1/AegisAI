    ...
    <div className="h-full flex flex-col relative">
      ...
      <div className="px-8 pt-5 pb-0 shrink-0">
        ...
        <div className="flex gap-8 text-xs mt-5 border-b border-black/10">
          {['Overview', 'Trace', 'Proposed Fix', 'Integrations', 'Timeline'].map(tab => (
            <div 
              key={tab}
              onClick={() => setActiveTab(tab)}
              className={`pb-3 cursor-pointer ${activeTab === tab ? 'font-bold text-black border-b-2 border-black' : 'text-black/50 hover:text-black'}`}
            >
              {tab}
            </div>
          ))}
          ...
          <div className="flex-1 max-w-2xl">
            ...
            {activeTab === 'Trace' && (
              <div className="mb-10">
                <div className="flex justify-between items-center mb-6">
                  ...
                  <span className="text-[10px] text-black/40 font-mono">Trace ID: {incident.trace_ref || 'tr_a94bf20'}</span>
                </div>
                
                <div className="relative border-l-2 border-black/10 ml-3 pl-6 space-y-8">
                  <div className="relative">
                    <div className="absolute -left-[31px] bg-[#dde5dd] p-1 rounded-full"><div className="w-2 h-2 bg-black rounded-full"></div></div>
                    <div className="text-xs font-bold mb-1">Agent Initialized</div>
                    <div className="text-[10px] font-mono text-black/50 bg-white/40 p-2 rounded-lg border border-black/5 mt-2 overflow-x-auto">
                      {`{"task": "Run data sync", "context": {"user_id": 1928}}`}
                    </div>
                  </div>
                  
                  <div className="relative">
                    <div className="absolute -left-[31px] bg-[#dde5dd] p-1 rounded-full"><div className="w-2 h-2 bg-emerald-500 rounded-full"></div></div>
                    <div className="text-xs font-bold mb-1">LLM Inference <span className="text-[9px] font-normal text-black/40 ml-2">gpt-4-turbo • 842ms</span></div>
                    <div className="text-xs text-black/70 mt-1">Calling tool: <span className="font-mono text-[10px] bg-black/5 px-1 py-0.5 rounded">fetch_api</span></div>
                  </div>
                  
                  <div className="relative">
                    <div className="absolute -left-[31px] bg-[#dde5dd] p-1 rounded-full"><div className="w-2 h-2 bg-red-500 rounded-full shadow-[0_0_8px_rgba(239,68,68,0.5)]"></div></div>
                    <div className="text-xs font-bold mb-1 text-red-700">Execution Halted</div>
                    <div className="text-[10px] font-mono text-red-900/80 bg-red-100/50 p-2 rounded-lg border border-red-200 mt-2">
                      {incident.root_cause || "Error: Missing required parameter 'timeout'"}
                    </div>
                  </div>
                </div>
                ...
                {incident.evidence && incident.evidence.length > 0 && (
                  <div className="mt-8">
                    <h4 className="text-xs font-bold mb-3">Raw Evidence Logs</h4>
                    <div className="bg-[#1a1a1a] text-white/80 p-4 rounded-xl text-[10px] font-mono overflow-x-auto">
            ...
            {activeTab === 'Integrations' && (
              <div className="mb-10">
                <h3 className="text-sm font-bold flex items-center gap-2 mb-6">
                  ...
                  Incident Dispatch Status
                </h3>
                
                <div className="grid grid-cols-1 gap-3">
                  <div className="flex items-center justify-between p-4 bg-white/40 border border-black/5 rounded-xl">
                    <div className="flex items-center gap-3">
                      <div className="w-8 h-8 rounded bg-black/5 flex items-center justify-center">
                        <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M13 10V3L4 14h7v7l9-11h-7z"></path></svg>
                      </div>
                      <div>
                        <div className="text-xs font-bold">Slack</div>
                        <div className="text-[10px] text-black/50">#aegis-incidents</div>
                      </div>
                    </div>
                  ...
                  <div className="flex items-center justify-between p-4 bg-white/40 border border-black/5 rounded-xl">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <div className="text-[10px] text-black/50">palakbhatt1/AegisAI</div>
                      </div>
                    </div>
                    <div className="flex items-center gap-2 text-xs font-bold text-emerald-600">
                      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M5 13l4 4L19 7"></path></svg>
                  ...
                  <div className="flex items-center justify-between p-4 bg-white/40 border border-black/5 rounded-xl">
                    <div className="flex items-center gap-3">
                      <div className="w-8 h-8 rounded bg-black/5 flex items-center justify-center">
                        <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8l-6-6zm-1 1.5L18.5 9H13V3.5zM17 18H7v-2h10v2zm0-4H7v-2h10v2zm-3-4H7V8h7v2z"></path></svg>
                      </div>
                      <div>
                        <div className="text-xs font-bold">Google Sheets</div>
                        <div className="text-[10px] text-black/50">Aegis Incidents Log</div>
                      </div>
                    </div>
                    <div className="flex items-center gap-2 text-xs font-bold text-emerald-600">
                      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M5 13l4 4L19 7"></path></svg>
            ...
            {activeTab === 'Timeline' && (
              <div className="mb-10">
                ...
                <div className="relative border-l-2 border-black/10 ml-3 pl-6 space-y-6">
                  <div className="relative">
                    <div className="absolute -left-[31px] bg-[#dde5dd] p-1 rounded-full"><div className="w-2 h-2 bg-black rounded-full"></div></div>
                    <div className="text-[10px] text-black/50 font-mono mb-1">14:32:05 UTC</div>
                    <div className="text-xs font-bold">Anomaly Detected</div>
                    <div className="text-xs text-black/70">Aegis intercepted a tool execution failure from {incident.agent_id}</div>
                  </div>
                  
                  <div className="relative">
                    <div className="absolute -left-[31px] bg-[#dde5dd] p-1 rounded-full"><div className="w-2 h-2 bg-black rounded-full"></div></div>
                    <div className="text-[10px] text-black/50 font-mono mb-1">14:32:08 UTC</div>
                    <div className="text-xs font-bold">Root Cause Diagnosed</div>
                    <div className="text-xs text-black/70">Automated diagnostic pipeline generated fix proposal</div>
                  </div>
                  
                  <div className="relative">
                    <div className="absolute -left-[31px] bg-[#dde5dd] p-1 rounded-full"><div className="w-2 h-2 bg-black rounded-full"></div></div>
                    <div className="text-[10px] text-black/50 font-mono mb-1">14:32:10 UTC</div>
                    <div className="text-xs font-bold">Notifications Dispatched</div>
                    <div className="text-xs text-black/70">Payloads successfully sent to Slack, GitHub, and Google Sheets</div>
                  </div>
                  
                  {incident.status !== 'notified' && incident.status !== 'under_review' && (
                    <div className="relative">
                      <div className="absolute -left-[31px] bg-[#dde5dd] p-1 rounded-full">
                        <div className={`w-2 h-2 rounded-full ${incident.status === 'approved' || incident.status === 'resolved' ? 'bg-emerald-500' : 'bg-red-500'}`}></div>
                      </div>
                      <div className="text-[10px] text-black/50 font-mono mb-1">14:35:12 UTC</div>
                      <div className="text-xs font-bold capitalize">Decision: {incident.status}</div>
                      <div className="text-xs text-black/70">A human operator submitted a decision via the dashboard</div>
                    </div>
                  )}
                  ...
                  {incident.status === 'resolved' && (
                    <div className="relative">
                      <div className="absolute -left-[31px] bg-[#dde5dd] p-1 rounded-full"><div className="w-2 h-2 bg-emerald-500 rounded-full"></div></div>
                      <div className="text-[10px] text-black/50 font-mono mb-1">14:35:45 UTC</div>
                      <div className="text-xs font-bold text-emerald-700">Incident Resolved</div>
                      <div className="text-xs text-black/70">Fix was successfully applied and verified by the test pipeline</div>
                    </div>
                  )}
                </div>
              </div>
